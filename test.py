...
env = gym.make('gym_carla:carla-v0',args=args, render_mode="rgb_array")
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            if terminated:
                ...
                next_state = torch.tensor(np.append(observation["obstacles"],[observation["speed"],observation["distTarget"]]), device=device).unsqueeze(0)
                #next_state = torch.tensor(np.append(np.delete(observation["obstacles"],[-1,-2],None),[observation["speed"],observation["distTarget"]]), device=device).unsqueeze(0) 
                #next_state = torch.tensor(np.array([observation["obstacles"],observation["speed"],observation["distTarget"]]), device=device).unsqueeze(0)
